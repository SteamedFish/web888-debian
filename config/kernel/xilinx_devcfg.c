...
/*
 * 6.6 port: the old xlnx trees exported zynq_slcr_init_preload_fpga() /
 * zynq_slcr_init_postload_fpga() from arch/arm/mach-zynq/slcr.c. Those
 * exports are gone (SLCR is a syscon/regmap now), so the SLCR writes are
 * replicated here via the regmap this driver already looks up in probe.
 */
...
static int xdevcfg_drv_probe(struct platform_device *pdev)
{
	...
	/* 6.6 port: pristine never assigned drvdata->dev, but the write/read
	 * paths pass it to dma_alloc_coherent()/dma_free_coherent(), which
	 * reject a NULL device on modern kernels. */
