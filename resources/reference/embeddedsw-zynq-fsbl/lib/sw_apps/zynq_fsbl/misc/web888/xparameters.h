...
*
* Web-888 board variant, adapted from misc/zc702/xparameters.h.
* Peripheral set and clock constants match the stock Web-888 FSBL's
* decoded SLCR programming (see docs/research/ps7-init-stock-analysis.md):
*   PS_CLK 33.333 MHz, IO PLL 1000 MHz
*   UART_CLK_CTRL  = 0x0A03  -> UART0+UART1, /10 -> 100 MHz
*   SDIO_CLK_CTRL  = 0x0A01  -> SDIO0,       /10 -> 100 MHz
*   GEM0_CLK_CTRL  = 0x00100801, RCLK /1    -> 125 MHz (gigabit)
*   PCAP_CLK_CTRL  = 0x501                  -> 200 MHz
*   CPU 666.7 MHz (6:2:1), TTC on CPU_1x = 111.1 MHz
******************************************************************************/
...
/* STDIN/STDOUT on UART0 (0xE0000000); stock MIO mux exposes UART0
 * on pins 14/15 and UART1 on pins 8/9. */
...
/* Definitions for peripheral PS7_DDR_0 (512 MiB on Web-888) */
...
/* SDIO_CLK_FREQ_HZ 100 MHz per decoded SDIO_CLK_CTRL=0x0A01 (IO PLL /10).
 * Card-detect is not wired to an SDIO MIO pin on Web-888 (handled as GPIO);
 * WP is on MIO47. HAS_CD/HAS_WP are not consumed by the sdps/xilffs code. */
...
/* Definitions for driver TTCPS (TTC clock = CPU_1x = 111.1 MHz) */
...
/* Definitions for driver UARTPS (both UARTs at 100 MHz per
 * decoded UART_CLK_CTRL=0x0A03) */
